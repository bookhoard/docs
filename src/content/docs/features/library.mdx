...
- The **Currently Reading** smart shelf, in the sidebar, lists every book
  with progress — see [Shelves](/features/shelves#smart-shelves)
...
- Add tags and a series (name plus a position, like `#3`) from a book's
  **Edit** drawer — type a tag and press Enter or `,` to add it
- Both are optional and shown on the book's detail panel; a series shows
  as `Name · #position`
- Filter the library grid by tag from the **Tags** menu above it — pick
  more than one and a book needs *all* of them to match
- The tag filter isn't saved between visits, so it can never make your
  whole library look empty just because you filtered it and left
