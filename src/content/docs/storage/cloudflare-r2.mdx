---
title: Cloudflare R2
description: Store books in Cloudflare R2 — over the S3-compatible API, or a native binding on Workers.
sidebar:
  label: Cloudflare R2
  order: 3
---
...
| --- | --- | --- |
| `STORAGE_DRIVER` | Yes | `s3` — the native `r2` binding mode only works when Bookhoarder itself runs on Cloudflare Workers |
...
| `S3_PREFIX` | No | Scope Bookhoarder's data under a subdirectory of a shared bucket |
...
| `R2_BUCKET_BINDING` | No | Defaults to `BOOKHOARD_BUCKET` — must match the `binding` name in `wrangler.jsonc`'s `r2_buckets` |
| `S3_PREFIX` | No | Scope Bookhoarder's data under a subdirectory of a shared bucket |
